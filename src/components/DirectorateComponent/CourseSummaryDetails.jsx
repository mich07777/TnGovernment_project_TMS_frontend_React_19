...
const handlePublish = () => {
  navigate('/PublishCourseDetails', { state: { courseId } });
};
  ...
  useEffect(() => {
    ...
    if (!dropdowns?.doctype?.length) return;

    async function fetchCourse() {
      try {
        const response = await axios.get(`${BASE_URL}/outside-courses/${courseId}?include_documents=true`);
        ...
        const parsedCourseData = {
          ...
          mode_of_receiving: courseData.mode_of_receiving || '',
          from_date_receiving: courseData.from_date_receiving || '',
          dated: courseData.dated || '',
          received_on: courseData.received_on || '',
          last_date: courseData.last_date || '',
          internal_cut_off_date: courseData.internal_cut_off_date || '',
          contact_person_name: courseData.contact_person_name || '',
          phone_number: courseData.phone_number || '',
          whatsapp_number: courseData.whatsapp_number || '',
          email_id: courseData.email_id || '',
        };

        setCourse(parsedCourseData);
        setEditedCourse(parsedCourseData);

        const processedDocuments = (courseData.documents || []).map(doc => ({
          ...doc,
          document_name: doc.document_name || doc.name || 'Unnamed Document',
          document_type_id: doc.document_type_id || doc.document_type?.id || doc.document_type,
          reference_number: doc.reference_number || '-',
          file_name: doc.file_name || doc.document_path?.split('/').pop() || ''
        }));
    ...
    fetchCourse();
  }, [courseId, dropdowns?.doctype]);

  const getTrainingCenterName = (id) => {
    const center = dropdowns.trainingcenter?.find(tc => tc.id == id);
  ...
  const handleSave = async () => {
    try {
      ...
      formData.append('internal_cut_off_date', formatForSubmit(editedCourse.internal_cut_off_date));
      formData.append('location', editedCourse.location || '');
      formData.append('training_center', editedCourse.training_center || '');
      ...
      formData.append('mode_of_receiving', editedCourse.mode_of_receiving || '');
      formData.append('from_date_receiving', editedCourse.from_date_receiving || '');
      formData.append('dated', editedCourse.dated || '');
      formData.append('received_on', editedCourse.received_on || '');
      ...
      formData.append('rank', JSON.stringify((editedCourse.rank || []).filter(r => r && r.id).map(r => r.id)));
      formData.append('qualification', JSON.stringify((editedCourse.qualification || []).filter(q => q && q.id).map(q => q.id)));
      formData.append('publish_to', JSON.stringify((editedCourse.publish_to || []).filter(p => p && p.id).map(p => p.id)));
      ...
      const response = await axios.put(`${BASE_URL}/outside-courses/${courseId}`, formData, {
        headers: {
      ...
      if (response.data.status === 'success') {
        const courseResponse = await axios.get(`${BASE_URL}/outside-courses/${courseId}?include_documents=true`);
        const raw = courseResponse.data.data || courseResponse.data;
        ...
        const updatedCourse = {
          ...
          location: raw.location || editedCourse.location || null,
          mode_of_receiving: raw.mode_of_receiving || '',
          from_date_receiving: raw.from_date_receiving || '',
          dated: raw.dated || '',
        ...
        const updatedDocs = (raw.documents || []).map(doc => ({
          ...doc,
          document_name: doc.document_name || doc.name || 'Unnamed Document',
          document_type: dropdowns.doctype.find(dt => dt.id == doc.document_type_id) ||
            { id: doc.document_type_id, name: 'Unknown' },
          reference_number: doc.reference_number || '-',
          file_name: doc.file_name || doc.document_path?.split('/').pop() || ''
        }));
  ...
  setEditedDocument({
    document_name: document.document_name || document.name || '',
  ...
  const handleUpdateDocument = async () => {
    try {
      ...
      const response = await axios.put(
        `${BASE_URL}/outside-courses/${courseId}/documents/${editingDocumentId}`,
        formData,
        ...
        const updatedDocuments = documents.map(doc =>
          doc.id === editingDocumentId
            ? {
              ...
              document_type:
                dropdowns.doctype?.find(dt => dt.id.toString() === editedDocument.document_type_id.toString()) ||
                { id: editedDocument.document_type_id, name: 'Unknown' },
              reference_number: editedDocument.reference_number,
              file_name: editedDocument.file ? editedDocument.file.name : doc.file_name
            }
            : doc
        );
...
const handleDeleteDocument = async (docId) => {
  ...
  try {
    const response = await axios.delete(`${BASE_URL}/outside-courses/documents/${docId}`);
...
const renderEditableField = (fieldName, value) => {
  if (isEditing) {
    const isDateField = ['from_date_receiving', 'dated'].includes(fieldName);

    return (
      <Form.Control
        type={isDateField ? 'date' : 'text'}
        value={isDateField ? formatDateToInput(value) : value || ''}
        onChange={(e) =>
          setEditedCourse({
            ...editedCourse,
  ...
  const isDateField = ['from_date_receiving', 'dated'].includes(fieldName);
  const formattedValue = isDateField && value ? formatDateToInput(value) : value;
  ...
  const renderEditableSelect = (field, value = [], options = []) => {
    if (!isEditing) {
      ...
      return value.map((item, index) => item?.name || `Unknown-${index}`).join(', ');
    }

    const safeOptions = options.filter(Boolean).map(item => ({
      value: item.id,
      label: item.name
    }));

    const selectedValues = (value || []).filter(Boolean).map(item => ({
      value: item.id,
  ...
  const renderEditableDate = (field, value) => {
    const maxDate = (() => {
      ...
      if (field === 'internal_cut_off_date') {
...
const getDocumentTypeName = (doc) => {
  ...
  if (doc.document_type_id && dropdowns.doctype) {
    const found = dropdowns.doctype.find(dt => dt.id == doc.document_type_id);
    if (found) return found.name;
  }
  
  if (typeof doc.document_type === 'string') {
            ...
            <div className="card bg-white">
              <div className="card-header d-flex justify-content-between align-items-center">
                <h4 className='title-clr'>
                  <i className="bi bi-table me-2"></i> Summary of Outside State Course: {course.course_name}
                </h4>
                {isEditing ? (
                  <div>
                    ...
                    <Card.Body>
                      ...
                      <Row className="mb-3">
                        ...
                        <Col md={4}>
                          ...
                          <div>{renderEditableDate('internal_cut_off_date', editedCourse?.internal_cut_off_date || '')}</div>
                        </Col>
                        <Col md={4}>
                      ...
                      <Row className="mb-3">
                        <Col md={4}>
                          <h6 className='txtred'>Mode of Receiving: </h6>
                          <div>
                            {isEditing ? (
                              <Form.Select
                                value={editedCourse.mode_of_receiving || ''}
                                onChange={(e) => handleInputChange('mode_of_receiving', e.target.value)}
                              >
                                <option value="">Select Mode</option>
                                {(dropdowns.receivingmode || dropdowns.modeofreceiving || []).map(mode => (
                                  <option key={mode.id} value={mode.id}>{mode.name}</option>
                                ))}
                              </Form.Select>
                            ) : (
                              (dropdowns.receivingmode || dropdowns.modeofreceiving || [])
                                .find(m => m.id == editedCourse.mode_of_receiving)?.name || '-'
                            )}
                          </div>
                        </Col>
                        <Col md={4}>
                          <h6 className='txtred'>From (Receiving): </h6>
                          <div>{renderEditableField('from_date_receiving', editedCourse.from_date_receiving)}</div>
                        </Col>
                        <Col md={4}>
                          <h6 className='txtred'>Dated: </h6>
                          <div>{renderEditableField('dated', editedCourse.dated)}</div>
                        </Col>
                      </Row>

                      <Row className="mb-3">
                        <Col md={4}>
                          <h6 className='txtred'>Received On: </h6>
                          <div>{renderEditableDate('received_on', editedCourse.received_on)}</div>
                        </Col>
                        <Col md={4}>
                          ...
                          <div>
                            {isEditing ? (
                              <Form.Select
                                ...
                                onChange={(e) => handleInputChange('location', e.target.value ? Number(e.target.value) : null)}
                              >
                              ...
                              dropdowns.locations?.find(loc => loc.id === editedCourse.location)?.name || '-'
                            )}
                          </div>
                        </Col>
                      </Row>
                      ...
                      <Row className="mb-3">
                        <Col md={4}>
                          <h6 className='txtred'>Training Center: </h6>
                          <div>
                            {isEditing ? (
                              <Form.Select
                                value={editedCourse.training_center || ''}
                                onChange={(e) => handleInputChange('training_center', e.target.value)}
                              >
                                <option value="">Select Training Center</option>
                                {dropdowns.trainingcenter?.map(center => (
                                  <option key={center.id} value={center.id}>{center.name}</option>
                                ))}
                              </Form.Select>
                            ) : (
                              getTrainingCenterName(editedCourse.training_center) || '-'
                            )}
                          </div>
                        </Col>
                        <Col md={4}>
...
export default OutsideStateCourse;